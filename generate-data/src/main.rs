...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    (0..=max_speed_r).into_par_iter().for_each(|speed_r| {
        ((-MAX_X / POS_FACTOR as i32)..=(MAX_X / POS_FACTOR as i32))
            .into_par_iter()
            .for_each(|x_r| {
                ((-MAX_Y / POS_FACTOR as i32)..=(MAX_Y / POS_FACTOR as i32))
                    .into_par_iter()
                    .for_each(|y_r| {
                        (0..YAW_FACTOR as i32).into_par_iter().for_each(|yaw_r| {
                            ...
                            .plan
                            {
                                ...
                                println!(
                                    "Done: x: {}, y: {}, local_vy: {}, yaw: {}",
                                    player.position.x,
                                    player.position.y,
                                    speed_r as f32 * SPEED_FACTOR,
                                    yaw
                                );
                            } else {
                                println!(
                                    "Failed: x: {}, y: {}, local_vy: {}, yaw: {}",
                                    player.position.x,
                                    player.position.y,
                                    speed_r as f32 * SPEED_FACTOR,
                                    yaw
